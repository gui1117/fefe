...
pub struct VelocityToPlayerInSightSystem;

impl<'a> System<'a> for VelocityToPlayerInSightSystem {
    type SystemData = (
        ReadStorage<'a, ::component::Player>,
        ReadStorage<'a, ::component::RigidBody>,
        WriteStorage<'a, ::component::VelocityToPlayerInSight>,
    ...
    fn run(&mut self, (players, rigid_bodies, mut vtpms, update_time, bodies_map, mut physic_world): Self::SystemData) {
        ...
        for (vtpm, rigid_body) in (&mut vtpms, &rigid_bodies).join() {
            ...
            if vtpm.next_refreash <= 0.0 {
                vtpm.next_refreash = ::component::VELOCITY_TO_PLAYER_IN_SIGHT_REFREASH_RATE;
                ...
                if let Some(closest_in_sight) = closest_in_sight {
                    let d = ::component::VELOCITY_TO_PLAYER_DISTANCE_TO_GOAL;
                    let v = (closest_in_sight - position).try_normalize(d).unwrap_or(::na::zero()) * vtpm.velocity;
